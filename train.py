import sys
import os
# 将当前文件所在目录的【上一级目录】（项目根目录）加入 Python 环境变量
# __file__ 是 train.py
# 第一次 dirname 得到 /data/user2/tmp/CONE/cone
# 第二次 dirname 得到 /data/user2/tmp/CONE
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        timer_start = time.time()
        # 【新增】：将归一化后的 span_labels 传入模型，供扩散模块前向加噪使用
        if targets is not None and "span_labels" in targets:
            pos_model_inputs["gt_spans"] = targets["span_labels"]

        pos_outputs = model.forward(**pos_model_inputs)
        if targets is not None and "span_labels" in targets:
            match_span_key = getattr(criterion, "match_span_key", None)
            if match_span_key == "pred_spans_refined" and match_span_key not in pos_outputs:
                raise KeyError(
                    f"Criterion expects `{match_span_key}` for matching, but model outputs only have: "
                    f"{sorted(list(pos_outputs.keys()))}"
                )
        if opt.neg_loss:
...
def start_training():
    ...
    train_data_ratio = getattr(opt, "train_data_ratio", opt.data_ratio)
    # Keep eval on full set by default so submission qids always match official GT.
    eval_data_ratio = 1.0
    logger.info(f"Data ratio setup => train: {train_data_ratio}, eval: {eval_data_ratio}")
    ...
    if opt.eval_path is not None:
        ...
        dataset_config["data_ratio"] = eval_data_ratio
