...
class HungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, span_key: str = "pred_spans"):
        ...
        """
        if span_key not in outputs:
            span_key = "pred_spans"
        if span_key not in outputs:
            raise KeyError(f"No span tensor found for matching. Tried span_key={span_key} and 'pred_spans'.")

        bs, num_queries = outputs[span_key].shape[:2]
        ...
        # 预测越准 → 概率越高 → cost 越小  - 号表示：置信度越高，代价越低
        if self.span_loss_type == "l1":
            # We flatten to compute the cost matrices in a batch
            out_spans = outputs[span_key].flatten(0, 1)  # [batch_size * num_queries, 2]
            ...
            # 交并比越高 → 效果越好 → cost越小所以加 - 号
        else:
            pred_spans = outputs[span_key]  # (bsz, #queries, max_v_l * 2)
