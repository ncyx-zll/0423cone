...
class StartEndDataset(Dataset):
    ...
    def get_span_labels(self, windows, ctx_l):
        ...
        if self.span_loss_type == "l1":
            windows = torch.Tensor(windows) / ctx_l  # normalized windows in xx# [start, end] 归一化到局部窗口
            # 【新增：扩散模型必需的严格截断】
            # 确保无论长视频中的动作跨度多大，在这 48 秒的局部窗口内，
            # 时间戳绝对不会越界，完美符合高斯噪声的定义域
            windows = torch.clamp(windows, min=0.0, max=1.0)

            windows = span_xx_to_cxw(windows)  # normalized windows in cxw# 转换为 [t_center, width]，即 1D 的参考点形式
...
def start_end_collate(batch):
    ...
    for k in model_inputs_keys:
        ...
        if k in ["saliency_pos_labels", "saliency_neg_labels"]:
            # 用 np.array 先堆叠，再转 Tensor，避免逐元素 Python 循环
            arr = np.array([item[k] for e in batch for item in e['model_inputs']], dtype=np.int64)
            batched_data[k] = torch.from_numpy(arr).long()
            continue
    ...
    for k in model_clip_inputs_keys:
        ...
        if k in ["query_cls_feat"]:
            # 修复警告：先用 np.stack 在 C 侧堆叠 NumPy 数组，再一次性转为 Tensor
            # 避免 torch.FloatTensor(list_of_ndarrays) 触发逐元素慢拷贝
            arr = np.stack([item[k] for e in batch for item in e['model_clip_inputs']])
            batched_clip_data[k] = torch.from_numpy(arr).float()
            continue
