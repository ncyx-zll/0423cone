...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        # Model config
        parser.add_argument('--adapter_module', default="none", type=str, choices=["linear", "mlp", "none"],
                            help="adapter_module: linear=2-layer MLP, mlp=LayerNorm+ReLU+residual (stronger), none=disabled")
        ...
        parser.add_argument('--infer_query_init', default='coarse_logits', type=str,
                            choices=['randn', 'coarse_logits'],
                            help="Inference denoising query init: random gaussian or coarse DETR logits init")
        parser.add_argument('--infer_t_mode', default='single', type=str,
                            choices=['multi', 'single', 'double'],
                            help="Inference denoising schedule: multi-step, single-step, or double-step [k,0]")
        parser.add_argument('--infer_t_k', default=20, type=int,
                            help="k used by double-step schedule [k, 0]")
        parser.add_argument('--infer_sample_steps', default=10, type=int,
                            help="number of sampling steps when infer_t_mode=multi")
        parser.add_argument('--infer_coarse_start_t', default=30, type=int,
                            help="start_t used by coarse_logits + multi-step schedule")
        ...
        if isinstance(self, TestOptions):
            ...
            for arg in saved_options:  # use saved options to overwrite all BaseOptions args.
                if arg not in ["eval_path", "eval_split_name", "results_root",
                               ...
                               "resume", "resume_all", "no_sort_results", "infer_query_init",
                               "infer_t_mode", "infer_t_k", "infer_sample_steps", "infer_coarse_start_t"]:
